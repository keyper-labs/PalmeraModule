    ...
    GuardManager
{
        ...
        external
    {
        ...
        if (fallbackHandler != address(0)) internalSetFallbackHandler(
            fallbackHandler
        );
        ...
        returns (bool success)
    {
        ...
        require(
            gasleft() >= safeTxGas * 64 / 63.max(safeTxGas + 2500) + 500, "GS010"
        );
        // Use scope here to limit variable lifetime and prevent `stack too deep` errors
        {
            ...
            if (gasPrice > 0) {
                payment = handlePayment(
                    gasUsed, baseGas, gasPrice, gasToken, refundReceiver
                );
            }
            if (success) emit ExecutionSuccess(txHash, payment); else emit
        ...
        returns (uint256 payment)
    {
        ...
        address payable receiver =
            refundReceiver == address(0) ? payable(tx.origin) : refundReceiver;
        ...
        for (i = 0; i < requiredSignatures; i++) {
            ...
            if (v == 0) {
                ...
                require(
                    msg.sender
                        == currentOwner
                        || approvedHashes[currentOwner][dataHash]
                        != 0,
                    "GS025"
                );
            } else if (v > 30) {
